      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
           <div className="space-y-4">
              ...
              <p className="text-slate-500 font-medium text-lg max-w-xl">Browse our curated collection of authentic Indian groceries.</p>
           </div>
        ...
        <motion.div 
          ...
          whileInView="show"
          viewport={{ once: true }}
          className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-x-8 gap-y-12"
        >
           {categories.map((cat, i) => (
             <motion.div key={i} variants={item}>
               <Link 
                 href={`/category/${cat.name.split(' ')[0].toLowerCase()}`}
                 className="group block text-center space-y-5"
               >
                  <motion.div 
                     whileHover={{ scale: 1.05, y: -5 }}
                     className={`w-28 h-28 md:w-36 md:h-36 bg-slate-50 rounded-4xl shadow-sm border border-slate-100 flex items-center justify-center mx-auto transition-all duration-300 group-hover:shadow-premium group-hover:border-primary/20 group-hover:bg-white text-${cat.color}`}
                  >
                     {cat.icon}
                  </motion.div>
