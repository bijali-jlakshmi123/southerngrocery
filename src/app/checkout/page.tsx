...
import { ChevronRight, Truck, CreditCard, ShieldCheck, MapPin, User, ChevronLeft } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  if (!user) {
    return (
      <div className="section-container min-h-[70vh] flex flex-col items-center justify-center text-center">
         ...
         <h2 className="text-4xl font-heading font-black text-slate-900 mb-4">Registration Required</h2>
         <p className="text-slate-500 mb-8 max-w-sm font-medium">To keep your orders secure, only registered customers can place orders. Please log in to continue.</p>
         <div className="flex gap-4">
            <Link href="/login" className="btn-base btn-primary px-12">Login Now</Link>
            <Link href="/register" className="btn-base btn-outline px-12">Create Account</Link>
         </div>
      </div>
    );
  }
  ...
  const handlePlaceOrder = () => {
    toast.success(`🎉 Order placed with ${paymentMethod}! Southern Spices is on its way.`, {
      position: 'top-center',
      autoClose: 5000,
      theme: 'colored'
    });
  ...
  if (items.length === 0) {
    return (
      <div className="section-container min-h-[70vh] flex flex-col items-center justify-center text-center">
         <h2 className="text-4xl font-heading font-black text-slate-900 mb-4">Wait, Your cart is empty!</h2>
         <p className="text-slate-500 mb-8 font-medium">Add some authentic Kerala groceries before checking out.</p>
         <Link href="/" className="btn-base btn-primary">Back to Shopping</Link>
      </div>
    );
  }
        ...
        <div className="flex-1 space-y-12">
          ...
          <div className="bg-white p-6 rounded-4xl border border-slate-100 flex items-center justify-between shadow-sm relative overflow-hidden">
             ...
             <div className={`flex flex-col items-center gap-2 flex-1 transition-all ${step >= 1 ? 'text-primary' : 'text-slate-300 font-bold'}`}>
                ...
                <span className="text-[10px] font-black uppercase tracking-widest hidden sm:block">Information</span>
             </div>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="space-y-4">
                  <h2 className="text-3xl font-heading font-black text-slate-900 flex items-center gap-4">
                    <div className="w-12 h-12 bg-primary/5 rounded-2xl flex items-center justify-center text-primary"><User size={24} /></div>
                    Customer Information
                  </h2>
                  <p className="text-slate-500 font-medium">Already have an account? <Link href="/login" className="text-primary font-black underline">Log in</Link></p>
                </div>

                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Full Name</label>
                    <input type="text" placeholder="Ajai Ram" className="w-full px-8 py-5 rounded-3xl border-2 border-slate-50 bg-slate-50 focus:bg-white focus:border-primary transition-all outline-none font-bold" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Email Address</label>
                    <input type="email" placeholder="ajairam@example.com" className="w-full px-8 py-5 rounded-3xl border-2 border-slate-50 bg-slate-50 focus:bg-white focus:border-primary transition-all outline-none font-bold" />
                  </div>
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Phone Number</label>
                    <input type="tel" placeholder="+44 XXXX XXXXXX" className="w-full px-8 py-5 rounded-3xl border-2 border-slate-50 bg-slate-50 focus:bg-white focus:border-primary transition-all outline-none font-bold" />
                  </div>
                </div>

                <button onClick={() => setStep(2)} className="btn-base btn-primary !py-4 w-full sm:w-auto px-12 mt-4 shadow-glow flex items-center justify-center gap-3 group">
            ...
            {step === 2 && (
              ...
              >
                <div className="space-y-4">
                  <h2 className="text-3xl font-heading font-black text-slate-900 flex items-center gap-4">
                    <div className="w-12 h-12 bg-primary/5 rounded-2xl flex items-center justify-center text-primary"><Truck size={24} /></div>
                    Shipping Method
                  </h2>
                  <p className="text-slate-500 font-medium">Select how you'd like your Southern Spices delivered.</p>
                </div>
                ...
                <div className="space-y-4">
                  <label className="relative flex items-center gap-6 p-8 rounded-4xl border-2 border-primary bg-primary/5 cursor-pointer group shadow-sm">
                    <input type="radio" name="shipping" checked className="hidden" />
                    <div className="w-6 h-6 rounded-full border-4 border-primary flex items-center justify-center bg-white"><div className="w-2 h-2 rounded-full bg-primary" /></div>
                    <div className="flex-1">
                      <div className="font-heading font-black text-slate-900 text-lg">Standard UK Delivery</div>
                      <div className="text-sm text-slate-500 font-bold uppercase tracking-widest">3-5 Business Days</div>
                    </div>
                    <div className="text-xl font-heading font-black text-primary">£5.99</div>
                    {shipping === 0 && <div className="absolute top-0 right-10 translate-y-[-50%] bg-success text-white text-[10px] font-black px-4 py-1.5 rounded-full shadow-glow">FREE FOR YOU</div>}
                  </label>
                  ...
                  <label className="relative flex items-center gap-6 p-8 rounded-4xl border-2 border-slate-50 bg-white hover:border-slate-100 cursor-pointer group transition-all">
                    ...
                    <div className="flex-1">
                       <div className="font-heading font-black text-slate-900 text-lg">Next Day Express</div>
                       <div className="text-sm text-slate-500 font-bold uppercase tracking-widest">Order by 2pm for tomorrow</div>
                    </div>
                    <div className="text-xl font-heading font-black text-slate-400">£9.99</div>
                  </label>
                </div>

                <div className="flex items-center gap-4 mt-8">
                   <button onClick={() => setStep(1)} className="btn-base btn-outline !rounded-2xl !py-4 px-8 flex items-center gap-2 font-bold text-slate-400 active:scale-95 transition-all"><ChevronLeft size={18} /> Back</button>
                   <button onClick={() => setStep(3)} className="btn-base btn-primary !py-4 flex-1 shadow-glow active:scale-95 transition-all">Proceed to Payment</button>
                </div>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-6">
                   ...
                   >
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Pay with Card</span>
                   </button>
                   <button 
                   ...
                   >
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Fast Checkout</span>
                   </button>
                </div>
                   ...
                   {paymentMethod === 'stripe' ? (
                     ...
                     >
                        <div className="bg-slate-900 rounded-5xl p-10 text-white space-y-8 shadow-2xl relative overflow-hidden group">
                           ...
                           <div className="flex justify-between items-start relative z-10">
                              ...
                              <img src="/stripe.png" className="h-6 opacity-30 invert" alt="" />
                           </div>
                           <div className="space-y-6 relative z-10">
                              <div className="space-y-2">
                                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Card Number</label>
                                 <input type="text" placeholder="●●●●  ●●●●  ●●●●  ●●●●" className="w-full bg-slate-800 border-none rounded-2xl px-6 py-4 font-heading font-black text-2xl tracking-widest outline-none text-primary placeholder:opacity-20" />
                              </div>
                              <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Expiry Date</label>
                                   <input type="text" placeholder="MM/YY" className="w-full bg-slate-800 border-none rounded-2xl px-6 py-4 font-heading font-black text-lg outline-none text-white placeholder:opacity-20" />
                                </div>
                                <div className="space-y-2">
                                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">CVV</label>
                                   <input type="text" placeholder="***" className="w-full bg-slate-800 border-none rounded-2xl px-6 py-4 font-heading font-black text-lg outline-none text-white placeholder:opacity-20" />
                                </div>
                              </div>
                           </div>
                        </div>
                     ...
                     >
                        ...
                        <div className="text-center group-hover:scale-105 transition-all">
                           <div className="font-heading font-black text-slate-900 text-xl">The smarter way to pay</div>
                           <p className="text-slate-500 font-medium text-sm mt-1">You will be redirected to PayPal to complete your purchase safely.</p>
                        </div>
                        <button onClick={handlePlaceOrder} className="bg-[#ffc439] hover:bg-[#f2ba36] text-[#003087] font-black !py-4 px-16 rounded-full flex items-center gap-3 transition-all active:scale-95 shadow-lg uppercase tracking-widest text-sm">
                ...
                <div className="flex items-center gap-4 bg-success/5 p-6 rounded-3xl border border-success/10 text-success">
                   ...
                   <div className="text-xs font-bold leading-relaxed italic">The secure processing of your payment details is our #1 priority. We do not store your full card information on our servers.</div>
                </div>

                <div className="flex items-center gap-4 mt-8">
                   <button onClick={() => setStep(2)} className="btn-base btn-outline !rounded-2xl !py-4 px-8 font-bold text-slate-400 flex items-center gap-2"><ChevronLeft size={18} /> Back</button>
                   {paymentMethod === 'stripe' && (
                     <button onClick={handlePlaceOrder} className="btn-base btn-primary !py-4 flex-1 shadow-glow group">
        ...
        <div className="lg:w-96">
           <div className="sticky top-28 space-y-8">
              ...
              <div className="flex flex-col items-center gap-4 text-center">
                 ...
                 <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Joined by <span className="text-slate-900 font-black">2,400+</span> Customers in UK</p>
              </div>
           </div>
        </div>
      </div>
