...
import { Mail, Lock, Apple, ArrowRight, User, Eye, Phone, ChevronLeft } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-12">
           ...
           <h1 className="text-4xl font-heading font-black text-slate-900 leading-tight">Welcome <span className="text-primary italic">Back</span></h1>
           <p className="text-slate-500 font-medium mt-3">Log in to your account to continue shopping.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2 relative">
             ...
             <div className="text-right mt-2">
                <Link href="/forgot-password" className="text-[10px] font-black text-slate-400 hover:text-primary uppercase tracking-widest">Forgot Password?</Link>
             </div>
          </div>
          ...
          >
             {isLoading ? 'Processing...' : 'Sign In'} <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
          </button>
        </form>
        ...
        <div className="grid grid-cols-2 gap-4">
           ...
           <button className="flex items-center justify-center gap-3 py-4 border-2 border-slate-50 bg-white rounded-2xl hover:border-slate-100 font-bold active:scale-95 transition-all">
              <img src="https://upload.wikimedia.org/wikipedia/commons/f/fa/Apple_logo_black.svg" alt="Apple" className="w-4 h-4" /> Apple
           </button>
        </div>

        <p className="mt-12 text-center text-slate-500 font-medium">
           Don't have an account? <Link href="/register" className="text-primary font-black underline underline-offset-4 decoration-2">Create Account</Link>
        </p>
        ...
        <div className="mt-12 pt-8 border-t border-slate-100 flex justify-center">
           <Link href="/" className="flex items-center gap-2 text-xs font-black text-slate-300 hover:text-secondary uppercase tracking-widest transition-colors group">
              <ChevronLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to Southern Spices
           </Link>
        </div>
      </motion.div>
    </div>
