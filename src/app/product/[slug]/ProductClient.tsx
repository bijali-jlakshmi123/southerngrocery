...
import { ShoppingCart, Star, Heart, Plus, Minus, CheckCircle2, Truck } from 'lucide-react';
  ...
  return (
    <div className="section-container pt-8 pb-12">
      ...
      <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-12">
        ...
        <span>/</span>
        <Link href={`/category/${mainCategorySlug}`} className="hover:text-primary transition-colors">{mainCategory}</Link>
        <span>/</span>
        ...
        <div className="flex flex-col">
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-4">
               <span className="bg-primary/10 text-primary text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">{mainCategory}</span>
               <div className="flex items-center gap-1">
                  <Star size={14} className="fill-yellow-400 text-yellow-400" />
                  <span className="text-sm font-black text-slate-700">{product.average_rating || '4.8'}</span>
                  <span className="text-xs text-slate-400 font-bold">({product.rating_count || 0} reviews)</span>
               </div>
            </div>
          ...
          <div className="bg-slate-50 rounded-5xl p-8 mb-10 border border-slate-100 shadow-sm relative overflow-hidden">
             ...
             <div className="flex items-baseline gap-4 mb-8">
               <span className="text-4xl font-heading font-black text-primary">£{price.toFixed(2)}</span>
               {originalPrice > price && (
                 <span className="text-xl text-slate-300 line-through font-bold">£{originalPrice.toFixed(2)}</span>
               )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
             <div className="flex items-center gap-3 bg-white p-4 rounded-3xl border border-slate-100 shadow-sm group">
                ...
                <span className="text-xs font-bold text-slate-600 truncate">Premium Quality Guaranteed</span>
             </div>
             <div className="flex items-center gap-3 bg-white p-4 rounded-3xl border border-slate-100 shadow-sm group">
                <div className="w-8 h-8 bg-success/10 text-success rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-success group-hover:text-white transition-all shadow-sm"><CheckCircle2 size={16} /></div>
                <span className="text-xs font-bold text-slate-600 truncate">Authentic Kerala Stock</span>
             </div>
          </div>
      ...
      <div className="mb-24">
         <h3 className="text-2xl font-heading font-black text-slate-900 mb-8 border-b pb-4">Product Description</h3>
         <div 
