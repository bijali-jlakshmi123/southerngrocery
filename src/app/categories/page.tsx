...
import { ChevronRight, ArrowRight, Package, ArrowLeft } from "lucide-react";
import { getCategories } from "@/lib/woocommerce";
import CategoriesClient from "./CategoriesClient";

export default async function CategoriesPage() {
  // Canonical Departments as seen on the reference site
  const DEPARTMENTS = [
    {
    ...
    { name: "Oil & Ghee", slug: "oil-and-ghee", image: "/cat-oils.png" },
    {
      name: "Spices & Masala",
      slug: "spices-and-masala",
      image: "/cat-spices.png",
    },
    { name: "Frozen Foods", slug: "frozen-foods", image: "/cat-frozen.png" },
    {
      name: "Snacks & Sweets",
    ...
    { name: "Beverages", slug: "beverages", image: "/cat-beverages.png" },
    { name: "Kitchenware", slug: "kitchenware", image: "/cat-kitchenware.png" },
  ];

  // Fetch product counts for these departments if possible, or just use defaults
  const categories = DEPARTMENTS.map((dept) => ({
    id: dept.slug,
    name: dept.name,
    slug: dept.slug,
    count: 0, // We'll show real count in the filter view
    image: dept.image,
  }));
    ...
    <div className="bg-slate-50 min-h-screen pb-24">
      ...
      <section className="bg-white pt-12 pb-20 border-b border-slate-100 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-slate-400 hover:text-primary transition-colors text-sm font-black uppercase tracking-widest mb-12 group"
          >
            <ArrowLeft
              size={16}
          ...
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-slate-500 font-medium leading-relaxed">
              Discover the finest collection of Kerala and Indian groceries.
              From everyday staples to traditional delicacies, we bring the
              taste of home to your UK doorstep.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
            <div className="max-w-xl text-center lg:text-left">
              ...
              <p className="text-lg text-slate-400 font-medium leading-relaxed">
                Looking for a specific brand or item not listed here? Our
                inventory updates daily with new traditional arrivals.
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-6 shrink-0 w-full lg:w-auto">
              <Link
                href="/contact"
                className="btn-base btn-primary !py-5 px-12 text-lg"
              >
                Request an Item <ChevronRight size={20} />
              </Link>
              <Link
                href="/search"
                className="btn-base bg-white/5 border border-white/10 text-white hover:bg-white/10 !py-5 px-12 text-lg"
              >
                Try Advanced Search
              </Link>
            </div>
