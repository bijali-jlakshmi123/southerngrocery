...
export default function FilterClient({ initialProducts }: { initialProducts: any[] }) {
  ...
  const filteredProducts = initialProducts
    .filter(p => {
      const matchesSubCat = selectedSubCats.length === 0 || selectedSubCats.includes(p.subCategory || 'Other');
  ...
  const subCategories = Array.from(new Set(initialProducts.map(p => p.subCategory || 'Other'))).filter(Boolean);
  const brands = Array.from(new Set(initialProducts.map(p => p.brand || 'Other'))).filter(Boolean);
    ...
    <div className="flex flex-col lg:flex-row gap-12">
      ...
      <aside className="hidden lg:block w-72 space-y-10 sticky top-28 h-fit">
        <div className="bg-white p-8 rounded-4xl border border-slate-100 shadow-sm">
          <h3 className="text-lg font-heading font-black text-slate-900 mb-6 flex items-center gap-3 border-b pb-4 border-slate-50">
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setIsFilterMobileOpen(false)} className="p-2 bg-slate-50 rounded-xl text-slate-400">
