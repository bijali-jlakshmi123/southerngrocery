...
const consumerKey = process.env.WC_CONSUMER_KEY || "ck_dummy";
const consumerSecret = process.env.WC_CONSUMER_SECRET || "cs_dummy";

/**
 * Generic Fetch Wrapper for WooCommerce API
 */
async function woocommerceFetch(endpoint: string, params: Record<string, any> = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(`[WooCommerce Fetch Error] ${endpoint}: HTTP ${response.status} - Content-Type: ${contentType}`);
      return { data: null };
    }

    if (!contentType.includes("application/json")) {
        const text = await response.text();
        console.warn(`[WooCommerce Warning] Received non-JSON response from ${endpoint}. Content-Type: ${contentType}. Usually means a redirect or server error.`);
