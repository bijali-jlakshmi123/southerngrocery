...
const brandImages = [
  { slug: "coconut-oil", url: "https://www.keralataste.com/cdn/shop/products/Cocopure_800x.jpg" } // Using KTC / Keralataste coconut oil as fallback
];
    ...
    try {
        ...
        while (page <= 5) {
            ...
            for (const p of products) {
                if (p.slug.includes('coconut-oil') || p.name.toLowerCase().includes('coconut oil') || p.slug.includes('coconut')) {
                    console.log(`Restoring ${p.name} (${p.slug}) to original brand image...`);
                    await axios.put(`${baseUrl}/products/${p.id}`, {
                        images: [{ src: brandImages[0].url }]
                    }, {
